...
import {fetchCommentByCommentId, fetchCommentsByCommentActId} from "@/utils/models/comment/comment.action";
...
export default async function (props: PageProps<{actId:string}>) {
    ...
    const [session, comments, act, comment ]= await Promise.all([getSession(), fetchCommentsByCommentActId(commentActId), fetchActByActId(commentActId), fetchCommentByCommentId(commentId)]);
        ...
        <>
            ...
            <section className="bg-gray-100 py-8 mx-auto max-w-sm sm:max-w-[28rem] md:max-w-[40rem] lg:max-w-screen-md">
                <div className="container mx-auto px-4">
                    ...
                    {(commentProfileId && commentActId) && <EditCommentForm comment={comment}/>}
